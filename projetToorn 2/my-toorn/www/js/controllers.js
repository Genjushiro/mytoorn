...
starterApp.controller('TournamentCtrl', function($scope, $http, $ionicLoading, $ionicPopup){
  ...
  $scope.showParticipants = function(id) {
    ...
    $http({
      method: 'GET',
      url: 'https://api.toornament.com/v1/tournaments/'+id+'/participants?api_key=qwx4d8q14REjWzzM3GDExGOks5Eaj1vSg6Fdl9MaN5c'
    }).success(function(data, status, headers, config){
    ...
    var alertPopup = $ionicPopup.alert({
      title: 'Participants',
      template: '<ion-list>                                '+
      '  <ion-item ng-repeat="item in participants"> '+
      '    {{item.name}}                              '+
      '  </ion-item>                             '+
      '</ion-list>                               ',
      scope: $scope
...
starterApp.controller('TournamentIdCtrl', function($scope, $http, $ionicLoading, $stateParams, $window, $ionicPopup){

  $ionicLoading.hide();
  $scope.discipline_id = $stateParams.id;
  $scope.empty = false;
  var collection = [];
  $http({
    method: 'GET',
    ...
    angular.forEach(data, function(value, key){
     if (value.discipline == $scope.discipline_id)
...
starterApp.controller('DisciplineCtrl', function($scope, $http){
  var description = $http.get('data/description.json');
  description.success(function(data, status, headers, config){
    $scope.description = data;
  });
  description.error(function(data, status, headers, config){
    return status;
  });
...
starterApp.controller('LivesCtrl', function($scope, $http){
/*  var discipline = $http.get('https://api.toornament.com/v1/disciplines?api_key=qwx4d8q14REjWzzM3GDExGOks5Eaj1vSg6Fdl9MaN5c');
  discipline.success(function(data, status, headers, config){
    $scope.discipline = data;
  });
...
starterApp.controller('EvenementCtrl', function($scope, $http){
  /*  var discipline = $http.get('https://api.toornament.com/v1/disciplines?api_key=qwx4d8q14REjWzzM3GDExGOks5Eaj1vSg6Fdl9MaN5c');
   discipline.success(function(data, status, headers, config){
   $scope.discipline = data;
   });
